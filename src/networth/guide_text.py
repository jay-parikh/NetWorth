...
GUIDE_ROWS: list[tuple] = [
    ...
    ("kv", "Mergers & demergers", "old shares price as the new company at the right ratio; spun-off shares appear as their own row — costs and dates carried correctly. Your share count doesn't change in a demerger; you simply get the new company as well."),
    ("kv", "…and they keep arriving", "nobody publishes merger and demerger cost splits in a form a program can read, so we write them down for you — and every update quietly picks up the newly published ones. Nothing to type, no new version to install."),
